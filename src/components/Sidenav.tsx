...
export function SidebarItem({ icon, text, alert, url, action }: SideBarProps) {
  ...
  const isSideNavOpen = useUIStore((state) => state.isSideNavOpen);
  const activeClass =
    pathname === url ? 'rounded-xl bg-primary text-white' : '';
  ...
  return (
    <Link href={url ? url : ''} onClick={action ? () => action() : () => {}}>
      ...
      >
        ...
        {!isSideNavOpen && (
          <div
            className={`invisible absolute left-full ml-6 -translate-x-4 rounded-md bg-green-200 px-2 py-1 text-sm text-indigo-800 opacity-20 group-hover:visible group-hover:translate-x-0 group-hover:opacity-100`}
          >
            {text}
          </div>
        )}
